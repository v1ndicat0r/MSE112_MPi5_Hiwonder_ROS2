...
class BoardInterface(Node):
    ...
    def __init__(self):
        ...
        rclpy_thread = threading.Thread(target=self.rclpy_handler,args=self)
        rclpy_thread.start()

    def rclpy_handler(board_interface):
        rclpy.spin(board_interface)
        board_interface.destroy_node()
        rclpy.shutdown()
    ...
    def set_servo_position_deg(self,servo_id,target):
        ...
        if servo_id == 4 or servo_id == 3:
            if target > 90.0:
                target = 90.0
            if target < -90.0:
                target = -90.0
            self.set_servo_position(servo_id,int((target+90.0-180.0)*2000.0/180.0+500.0))
